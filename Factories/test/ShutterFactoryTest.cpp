...
BOOST_AUTO_TEST_CASE(setting_up_shutter_factory_and_get_get_shutterstate_dictionaries)
{
	...
	{
		shut_fac.messenger.printDebugMessage("item.name = ", item.name);
		shut_fac.messenger.printDebugMessage("item.cmi = ", item.cmi);
		shut_fac.messenger.printDebugMessage("item.state = ", item.state);
	}
}
