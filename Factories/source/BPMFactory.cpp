...
BPMFactory::BPMFactory(bool isVirtual)
{
	...
	hasBeenSetup = false;
	printDebugMessage("BPM Factory Constructed");
	isVirtual = isVirtual;
...
{
	printDebugMessage("BPMFactory Destructor Called");
	if (hasBeenSetup)
	{
		for (auto& bpm : bpmMap)
		{
...
bool BPMFactory::setup(const std::string& version)
{
	...
	for (auto& bpm : bpmMap)
	{
		...
		for (auto& pv : bpmPVStructs)
		{
			...
			{
				printMessage(bpm.first, " CANNOT CONNECT TO EPICS");
				hasBeenSetup = false;
				return hasBeenSetup;
			}
		}
...
std::map<std::string, BPM> BPMFactory::getAllBPMs()
{
	...
	{
		printDebugMessage("BPMS HAVE ALREADY BEEN CONSTRUCTED.");
	}
	return bpmMap;
}
...
std::string BPMFactory::getBPMName(const std::string& name)
{
	if (!hasBeenSetup)
	{
		printDebugMessage("Please call BPMFactory.setup(VERSION)");
	}
	else
	{
...
//{
//	debugMessagesOn();
//	printDebugMessage("BPM-FAC - DEBUG ON");
...
//{
//	printDebugMessage("BPM-FAC - DEBUG OFF");
//	debugMessagesOff();
...
//{
//	messagesOn();
//	printMessage("BPM-FAC - MESSAGES ON");
//	reader.messagesOn();
//	for (auto& bpm : bpmMap)
//	{
...
//{
//	printMessage("BPM-FAC - MESSAGES OFF");
//	messagesOff();
//	reader.messagesOff();
//	for (auto& bpm : bpmMap)
//	{
