...
boost::circular_buffer< std::vector< double > > BPMFactory::getDataVector(const std::string& name)
{
	if (!hasBeenSetup)
	{
		messenger.printDebugMessage("Please call BPMFactory.setup(VERSION)");
...
std::map<std::string, boost::circular_buffer< std::vector< double > > > BPMFactory::getDataVectors(const std::vector<std::string>& names)
{
	std::map<std::string, boost::circular_buffer< std::vector< double > > > datamap;
	for (auto name : names)
	{
		booststd::circular_buffer< std::vector< double > > datapv = bpmMap.find(name)->second.getDataVector();
		datamap[name] = datapv;
	}
	return datamap;
}
...
{
	boost::circular_buffer< double > xpvvec;
	xpvvec = getXPVBuffer(bpmName);
	boost::python::list newPyList = to_py_list(xpvvec);
	return newPyList;
}
...
{
	boost::circular_buffer< double > ypvvec;
	ypvvec = getYPVBuffer(bpmName);
	boost::python::list newPyList = to_py_list(ypvvec);
	return newPyList;
}
...
{
	boost::circular_buffer< double > qvec;
	qvec = getQBuffer(bpmName);
	boost::python::list newPyList = to_py_list(qvec);
	return newPyList;
}
...
boost::python::list BPMFactory::getXPVBuffer_Py(const std::string& bpmName)
{
	boost::circular_buffer< std::vector< double > > datavec;
	datavec = getDataBuffer(bpmName);
...
boost::python::dict BPMFactory::getYPVBuffers_Py(boost::python::list bpmNames)
{
	...
	std::vector<std::string> bpmNamesVector = to_std_vector<std::string>(bpmNames);
	ypvvals = getXPVBuffers(bpmNamesVector);
	boost::python::dict newPyDict = to_py_dict(ypvvals);
	return newPyDict;
}
...
boost::python::dict BPMFactory::getQBuffers_Py(boost::python::list bpmNames)
{
	...
	std::vector<std::string> bpmNamesVector = to_std_vector<std::string>(bpmNames);
	qvals = getXPVBuffers(bpmNamesVector);
	boost::python::dict newPyDict = to_py_dict(qvals);
	return newPyDict;
}
