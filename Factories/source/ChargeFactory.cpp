...
//	messenger = LoggingSystem(false, false);
	hasBeenSetup = false;
	LoggingSystem::printDebugMessage("Charge Factory Constructed");
	isVirtual = isVirtual;
...
{
	LoggingSystem::printDebugMessage("ChargeFactory Destructor Called");
...
bool ChargeFactory::setup(const std::string &version)
{
	...
	for (auto &charge : chargeMap)
	{
		...
		for (auto& pv : chargePVStructs)
		{
			...
			{
				LoggingSystem::printMessage(charge.first, " CANNOT CONNECT TO EPICS");
				hasBeenSetup = false;
				return hasBeenSetup;
			}
		}
...
std::map<std::string, Charge> ChargeFactory::getAllChargeDiagnostics()
{
	...
	{
		LoggingSystem::printDebugMessage("CHARGE DIAGNOSTICS HAVE ALREADY BEEN CONSTRUCTED.");
	}
	return chargeMap;
}
...
std::string ChargeFactory::getChargeDiagnosticName(const std::string& name)
{
	if (!hasBeenSetup)
	{
		LoggingSystem::printDebugMessage("Please call ChargeFactory.setup(VERSION)");
	}
	else
	{
